...
[**Watch the Project Demonstration Video Here**](https://drive.google.com/file/d/1sXqaiMDcQmNPP-KQqiXTrPr6joBTz6z5/view?usp=sharing)
...
## System Concepts & Screenshots

### System Concepts

- **Technology Stack**: The system is built on a decentralized microservices architecture, utilizing Docker for containerization and Apache Kafka for asynchronous event streaming. Microservices communicate via gRPC and Protocol Buffers to ensure structured data flows.
- **PII Exfiltration Attack Flows**: PII Exfiltration attacks are simulated by testing how data association leads to unintended leakage. The system tests for verbatim sequence memorization within the LLM model and evaluates online learning mechanisms during persistent attacks. The attacks use specific categories and procedures to extract PII without relying on traditional jailbreaks.
- **Jailbreak Attacks**: Jailbreak evaluation metrics and bypassing methodologies are used to measure how well the LLM withstands payload splitting, persona adoption, and context switching to bypass safety guardrails.

### Application Screenshots

**Authentication & Dashboard**
![Login Screen](images/image.png)
*The secure login and registration interface for the Red-Line platform.*

![Main Dashboard](images/image-13.png)
*The central dashboard providing an overview of total jobs, active runs, completed tests, and any detected breaches.*

**Configuring & Launching Attacks**
![Launch New Attack](images/image-14.png)
*The attack configuration interface displaying references for different attack vectors like Prompt Injection, PII Exfiltration, Jailbreak, and Hallucination.*

![Configuring PII Exfiltration](images/image-3.png)
*Setting up a new PII Exfiltration attack against a target LLM endpoint.*

**Monitoring & Execution Logs**
![Live Execution Logs](images/image-4.png)
*Real-time streaming logs detailing the execution of complex SAGA state machine workflows.*

![Detailed Transactions](images/image-7.png)
*Deep-dive into transaction logs, showing structured JSON payloads for each attack step and AI Judge observation.*

**Vulnerability Reports & Insights**
![Vulnerability Report Overview](images/image-9.png)
*High-level vulnerability report summarizing categories tested, breaches detected, and overall risk level.*

![Attempts & Responses](images/image-6.png)
*Detailed view of individual attack attempts, displaying the exact prompts sent and the LLM's responses, evaluated for safety.*

![AI Judge Insights](images/image-10.png)
*In-depth per-category analysis and actionable insights generated by the independent AI Judge agent.*

![Infrastructure Monitoring](images/image-5.png)
*Docker infrastructure view showing the active microservices, including the Kafka message broker, Redis cache, and specialized attack agents.*
